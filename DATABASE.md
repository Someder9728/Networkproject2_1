...
## ความสัมพันธ์
- rooms ➔ players (1:N, เส้นประ) CASCADE: เมื่อลบห้อง ข้อมูลผู้เล่นในห้องจะถูกลบทิ้งอัตโนมัติ (ON DELETE CASCADE) = rooms_room_id
- rooms ➔ votes_and_actions (1:N, เส้นประ) = rooms_room_id
- rooms ➔ chat_messages (1:N, เส้นประ) = rooms_room_id
- players ➔ votes_and_actions (1:N, เส้นประ 2 เส้น) = players_voter_id และ players_target_id
- players ➔ chat_messages (1:N, เส้นประ) = players_sender_id

## SQL DDL Script (สำหรับ Create Database)
-- Create Database
CREATE DATABASE IF NOT EXISTS werewolf_game DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE werewolf_game;
...
CREATE TABLE IF NOT EXISTS rooms (
    ...
    room_status ENUM('waiting', 'day_discussion', 'day_voting', 'night', 'ended') NOT NULL,
...
CREATE TABLE IF NOT EXISTS votes_and_actions (
    ...
    phase_type ENUM('night', 'day') NOT NULL,
    action_type ENUM('vote_lynch', 'werewolf_kill', 'seer_check') NOT NULL,
    players_voter_id INT NOT NULL,
    players_target_id INT NOT NULL,
    CONSTRAINT fk_votes_rooms
        FOREIGN KEY (rooms_room_id) REFERENCES rooms (room_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT fk_votes_voter_player
        FOREIGN KEY (players_voter_id) REFERENCES players (player_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT fk_votes_target_player
        FOREIGN KEY (players_target_id) REFERENCES players (player_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS chat_messages (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_chats_rooms
        FOREIGN KEY (rooms_room_id) REFERENCES rooms (room_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    CONSTRAINT fk_chats_sender_player
        FOREIGN KEY (players_sender_id) REFERENCES players (player_id)
        ON DELETE CASCADE
        ON UPDATE CASCADE
);